function [yprime,params,resnorm,residual,conf, func]=lorentzian_fit_mtx(x,y,dhyp,Nhyp,Npeak,smp_mtx,varargin)
...
for i=1:3:3*Npeak
    if Nhyp == 2
        ...
        funcString2 = [funcString2 sprintf('(p(%d)./(pi*p(%d)))*((1./(((-0.5*%d+p(%d)-x))./(0.5*p(%d))).^2+1)+(1./(((0.5*%d+p(%d)-x)./(0.5*p(%d))).^2+1)))+',...
           i+2, i+1, dhyp, i, i+1, dhyp, i, i+1)]; %#ok<AGROW>
    elseif Nhyp == 3
...
% for i=1:3:3*Npeak
%     if i==1
%         curr_kernel = @(p,x) p(i+2)*kernel(x,p(i+1),p(i));
%         func = @(p,x) curr_kernel(p,x);
%     else
%         curr_kernel = @(p,x) p(i+2)*kernel(x,p(i+1),p(i));
%         func = @(p,x) curr_kernel(p,x)+func(p,x);
%     end
% end
% func = @(p,x) func(p,x)+p(25);



mtx_func = @(p,x) (smp_mtx*(func(p,x)-mean(func(p,x)))')';%
% funcString = ' @(p,x)';
% for i=1:3:3*N
...
% % % w = ones(length(y),1)';
% % % w(y>0.0006) = 10;
% [params,residual,jacobian,covariance,MSE,ErrorModelInfo]=nlinfit(x,y,mtx_func,p0,options);

 % TRY LSQCURVEFIT
 bounds = zeros(length(p0),2);
 for i=1:length(p0)
     if i==length(p0)
         %offset near zero
         bounds(i,1) = -0.003;
         ...
         switch mod(i,3)
             case 0 %amplitude
                 bounds(i,1) = p0(i)-1*abs(p0(i));
                 bounds(i,2) = p0(i)+0.6*abs(p0(i));
             case 1 %peak location
                 bounds(i,1) = p0(i)-5;
                 bounds(i,2) = p0(i)+5;
             case 2 % width
                 bounds(i,1) = 8;
 ...
 lsq_options.TypicalX = p0;
 %lsq_options.FiniteDifferenceType = 'central';
 %lsq_options
[params,resnorm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(mtx_func,p0,x,y,bounds(:,1),bounds(:,2),lsq_options);
